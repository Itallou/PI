  ...
  return (
    <>
      ...
      {cards.map((card, index) => (
        <div key={card.id} className={`grid grid-cols-2 flex flex-col items-center mt-6 mb-6 w-[950px] rounded-lg pr-2 p-6 gap-10 justify-center shadow-lg ${index === cards.length - 1 ? 'ring ring-purple-700' : 'ring-[#FA023E] ring-2'}`}>
          <div className='w-[181%]'>
            {/* Ajuste na cor do h3 */}
            <h3 className={`${index === cards.length - 1 ? 'text-[#4B00E0]' : 'text-[#FA023E]'} text-[23px] font-medium mb-2`}>{card.title}</h3>
          ...
          <div className="flex w-[110px] ml-[325px]">
            ...
            <div className='flex flex-col gap-4 w-full'>
              ...
              {index === cards.length - 1 ? (
                <>
                <div className='flex flex-row gap-2  bg-[#00B7FF] rounded-lg'>
                  <button className='text-[#000000] text-[14px] p-1 ml-2 font-medium'>Adicionar Apresentador</button>
                </div>
